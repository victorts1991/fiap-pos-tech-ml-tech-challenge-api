...
def importacao(categoria):
    try:
        ...
        if categoria == 'vinhos_de_mesa':
            ...
            resultado_scraping = scraper.scrape_processamento_uvas_passas()
        elif categoria == 'suco_de_uva':
            resultado_scraping = scraper.scrape_processamento_suco_de_uva()
        else:
            return jsonify({"error": "Categoria inválida"}), 400
...
def comercializacao():
    try:
        ...
        return jsonify({"dados": dados_comercializacao})
    except Exception as e:
